...
class ClienteGameActivity : AppCompatActivity() {
    ...
    private val reGanhador = object : BroadcastReceiver() {
        override fun onReceive(context: Context?, intent: Intent?) {
            val resultado = intent?.getIntExtra("GANHADOR", 0)?: return

            when (resultado) {
                0 -> {
                    tVJogada.text = "Empate"
                }

                2 -> {
                    tVJogada.text = "Ganhou!"
                }

                1 -> {
                    tVJogada.text = "Perdeu!"
                }
        ...
        btJogada.setOnClickListener {
            if (estado) {
                if (jogada != Jogada.NADA) {
                    estado = false
                    val paraActServer = Intent("JogadaCliente").apply {
                ...
                val paraActServer = Intent("estadoCliente")
